...
if __name__ == "__main__":
    # Step 1: Load and merging two datasets AppGallery and Purchasing
    load_and_merge_datasets()

    # Step 2: Encode labels in the merged dataset
    encode_labels()

    # Step 3: Preprocess the text by cleaning and transforming it
    preprocess_text()

    # Step 4: Load the cleaned data and apply de-duplication
    ...
    # Step 5: Split into features and separate targets for each model
    X = df.drop(columns=["Type 1", "Type 2", "Type 3", "Type 4"])

    # Target variables
    # Chained Model targets
    ...
    X = X.drop(columns=["Ticket Summary"])  # Drop text column after vectorization
    ...
    # Step 6: Split dataset separately for each model
    ...
    # Train & Evaluate the Chained Model
